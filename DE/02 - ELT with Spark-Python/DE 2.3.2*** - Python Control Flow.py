...
# MAGIC %md Uncomment the following cell and then run it to assert that the string **`"2"`"** is an integer.
# MAGIC 
...
# MAGIC %md We can use the **`find()`** method to test for multiple SQL statements by looking for a semicolon.
...
# MAGIC %md If it's not found it will return **`-1`**
...
# MAGIC %md Uncomment the following cell and give it a try:
